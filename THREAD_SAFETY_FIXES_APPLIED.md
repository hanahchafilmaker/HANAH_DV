I have applied thread safety fixes to main_gui.py to eliminate the 'invalid command name' error when closing the footnote editor during processing. The changes include:
...
2. Implemented queue-based dispatcher pattern:
   - Worker thread (_process_all_footnotes_matching) performs auto-matching and puts results into self.ui.queue.
...
The thread safety fixes have been applied and the error should now be resolved.